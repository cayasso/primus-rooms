...
function Rooms(adapter, key) {
  if (!(this instanceof Rooms)) return new Rooms(adapter, key);
  this.key = key;
...
Rooms.prototype.exec = function exec(method, spark, room, fn) {
  ...
  for (var i = 0; i < len; ++i) {
    var _room = rooms[i];
    if (1 === len) return this[method](spark, _room, fn);
    this[method](spark, _room, response(_room, i));
  }
  ...
  function response(room, limit) {
    return function callback(err, res) {
      if (err) return fn(err, null);
      re = {};
      re[room] = res;
      result.push(re);
      if (len-1 === limit) fn(null, result);
    };
  }
