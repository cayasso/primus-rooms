...
Rooms.prototype._join = function _join(room, fn) {
  ...
  this.adapter.add(this.id, room, function add(err, res) {
...
Rooms.prototype._leave = function _leave(room, fn) {
  ...
  this.adapter.del(this.id, room, function add(err, res) {
    if (err) return fn(new RoomsError(err, rm.ctx));
...
Rooms.prototype.leaveAll = function leaveAll(fn) {
  var rm = this;
  fn = fn || noop;
  this.adapter.del(this.id, null, function del(err, res) {
    if (err) return fn(new RoomsError(err, rm.ctx));
    rm.emits('leaveallrooms');
    fn(null, res);
  });
  return this;
};
...
Rooms.prototype.emits = function (ev, data) {
  ...
  this.ctx.primus.emit(ev, data, this.ctx);
  return this;
}
...
Object.defineProperty(Rooms.prototype, 'connections', {
  get: function get() {
    return this.ctx.primus ?
      this.ctx.primus.connections :
