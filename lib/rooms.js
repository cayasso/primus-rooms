...
Rooms.prototype.broadcast = function (data, sparks) {
  return this._broadcast(this.spark.id, data, sparks);
};
...
Rooms.prototype._broadcast = function (id, data, sparks) {
...
Rooms.prototype.clients = function (room, fn) {
  ...
  if (room) {
    this._clients(null, oom, fn);
  } else {
    if (this._rms) {
      if (1 === this._rms.length) {
        this._clients(null, this._rms[0], fn);
      } else {
        exec.call(this, '_clients', null, this._rms, fn);
      }
      delete this._rms;
    } else {
      fn(null, []);
...
Rooms.prototype._clients = function (id, room, fn) {
...
Rooms.prototype.join = function(room, fn){
  return exec.call(this, '_join', this.spark.id, room, fn);
};
...
Rooms.prototype._join = function(id, room, fn){
  var rooms = this;
  debug('joining room %s', room);
  if (~rooms._rooms.indexOf(room)) return this;
  rooms.adapter.add(id, room, function(err){
...
Rooms.prototype.leave = function (room, fn) {
  return exec.call(this, '_leave', this.spark.id, room, fn);
};
...
Rooms.prototype._leave = function(id, room, fn){
  var rooms = this;
  debug('leave room %s', room);
  rooms.adapter.del(id, room, function(err){
    if (err) return fn && fn(err);
...
function exec(method, id, room, fn){

  var rooms = room, l, err = null, count = 0, result = [];
  if ('string' === typeof room) {
    rooms = room.split(' ');
    if (1 === rooms.length) {
      return this[method](id, rooms[0], fn);
    }
  }
